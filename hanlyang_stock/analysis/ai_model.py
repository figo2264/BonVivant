...
try:
    ...
    # 독립 실행 모드 - 필요한 모듈들을 직접 구현
    _STANDALONE_MODE = True
    
    # 독립 실행 시 필요한 데이터 소스 임포트
    try:
        import FinanceDataReader as fdr
        from pykrx import stock as pystock
    except ImportError as e:
        print(f"⚠️ 필수 라이브러리 누락: {e}")
        print("설치 명령: pip install FinanceDataReader pykrx")

warnings.filterwarnings('ignore')


# 독립 실행 모드를 위한 헬퍼 함수들
def _get_past_data_total_standalone(n=1000):
    """독립 모드에서 전체 시장 과거 데이터 조회 (pykrx 사용)"""
    try:
        all_data = []
...
def _create_technical_features_standalone(data):
    """독립 모드에서 강화된 기술적 분석 지표 생성"""
    try:
        if len(data) < 30:
            # 데이터가 부족하면 기본 지표만 생성
...
class StandaloneDataFetcher:
    ...
    def get_past_data_total(self, n=1000):
        return _get_past_data_total_standalone(n)
    
    def get_past_data_enhanced(self, ticker, n=50):
        """단일 종목 데이터 조회 (독립 모드)"""
        try:
            # 최근 데이터 가져오기
            end_date = datetime.now()
            start_date = end_date - timedelta(days=n*2)  # 충분한 데이터 확보
    ...
    def get_market_data_by_date(self, date_str):
        """특정 날짜의 시장 데이터 조회 (독립 모드)"""
        try:
            date_obj = datetime.strptime(date_str, '%Y-%m-%d')
            date_pykrx = date_obj.strftime('%Y%m%d')
            
            # KOSPI와 KOSDAQ 데이터 수집
            try:
                kospi = pystock.get_market_ohlcv(date_pykrx, market='KOSPI')
            except:
                kospi = pd.DataFrame()
            
            try:
                kosdaq = pystock.get_market_ohlcv(date_pykrx, market='KOSDAQ')
            except:
                kosdaq = pd.DataFrame()
...
class AIModelManager:
    ...
    def __init__(self):
        if _STANDALONE_MODE:
            self.data_fetcher = StandaloneDataFetcher()
        else:
    ...
    def train_ai_model_at_date(self, end_date):
        ...
        try:
            # 기존 train_ai_model의 로직을 그대로 사용하되 데이터만 교체
            return self._train_model_with_data(X, y, end_date)
        except Exception as e:
            print(f"❌ 모델 훈련 오류: {e}")
            return None
    
    def _train_model_with_data(self, X, y, train_date):
        """데이터로 모델 훈련 (내부 메서드)"""
        try:
            # 데이터 분할 (시계열 고려)
            n_samples = len(X)
    ...
    def prepare_training_data_until(self, end_date, lookback_days=1000):
        ...
        try:
            ...
            for ticker in historical_data['ticker'].unique():
                ...
                # 강화된 기술적 지표 생성
                if _STANDALONE_MODE:
                    ticker_data = _create_technical_features_standalone(ticker_data.copy())
                else:
    ...
    def get_ai_prediction_score(self, ticker: str, current_date=None, model=None) -> float:
        ...
        try:
            ...
            # 강화된 기술적 지표 생성
            if _STANDALONE_MODE:
                data = _create_technical_features_standalone(data)
            else:
...
def main():
    ...
    # AI 모델 매니저 생성 및 훈련 실행
    manager = get_ai_manager()
    model = manager.train_ai_model()
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("💡 이제 다른 모듈에서 새로운 모델을 사용할 수 있습니다.")
    else:
